...
# ╔═╡ 21bf237a-b425-46ac-8fd5-db5d66bf88c5
begin 
	using PlutoUI
	using Latexify
	TableOfContents()
...
# ╔═╡ 1cc9b12b-0cc6-45a0-aa70-9a1329f02e0d
begin
	using Statistics
...
# ╔═╡ 9427f034-6ffc-4f80-be03-144c7d82d637
md"""
...
# ╔═╡ 4f3661d4-fea1-4873-88fc-7bcfaccb5046
md"""
This is a slightly more complex learning task than the MNIST example.  
[CIFAR10](https://www.cs.toronto.edu/~kriz/cifar.html) is a dataset of 50k tiny coloured training images split into 10 classes.
...
# ╔═╡ 8791d668-0652-4c25-9065-ddad782df1ac
md"""
### Load the dataset
"""

# ╔═╡ cf7d7053-7482-4ec6-a850-e904da947ffd
md"""
The image gives an idea of the range of images in each of the 10 categories.
# ![title](https://pytorch.org/tutorials/_images/cifar10.png)
"""

# ╔═╡ 1a44d90c-972f-4c37-b6e0-2f79d89625cf
md"""
Again, we'll get the data from the MLDatasets repository. 
"""

# ╔═╡ a89cc0e6-3f65-4088-bb04-5bf5e66703b7
ENV["DATADEPS_ALWAYS_ACCEPT"] = true

# ╔═╡ e00fc925-b57c-48b9-ba62-49c9f51d8fdc
begin
...
# ╔═╡ 49e7f528-306e-46a3-8861-d413e3fb4e5a
md"""
The images are simply 32 x 32 matrices of numbers in 3 channels (R,G,B). The train\_x array contains 50,000 images converted to 32 x 32 x 3 arrays with the third dimension being the 3 channels (R,G,B). Let's take a look at a random image from train_x.  However, to do this we need to define a function called `image`, which calls `colorview` on the training image, which we have to permute from 32x32x3 to 3x32x32:
"""

# ╔═╡ e707879b-2403-42be-93f3-dbf61b87f368
image(x) = colorview(RGB, permutedims(x, (3, 2, 1)))

# ╔═╡ 310f1196-69da-4dc5-8602-58955768e3d3
begin
	rand_train = rand(1:size(train_x)[4])
	plot(image(train_x[:,:,:,rand_train]), axis=false)
	annotate!(-1.0, 1.0, text(classes[train_y[rand_train]+1], :blue, :right, 12))
end

# ╔═╡ 1209adfa-e8ab-4d90-bebc-7ce26b725b23
md"""
### Rehape data for training with `flux`
"""

# ╔═╡ 25794f09-7270-4c12-a3e8-a1bea9b494da
md"""
 We can now
arrange them into batches of 1,000.
...
# ╔═╡ 9650f8fb-981b-473f-b450-1686a7cbb9eb
md"""
!!! info "Task 1"
	Partition train_x into training and validation parts, along the lines done for the MNIST example.
"""

# ╔═╡ 9fc2cbc7-6ad1-4d69-aa00-5541c623dc70
md"""
## <your code here>
"""

# ╔═╡ 0269cd6f-9a18-4780-96d2-a70fb6eb5493
begin
    train = [(train_x[:, :, :, i], train_labels[:, i])
             for i in partition(1:49000, 1000)] 
    validate = [(train_x[:, :, :, i], train_labels[:, i])
                for i in partition(49001:50000, 1000)] 
end

# ╔═╡ fae9aff5-fbd9-4f0a-9622-e6a1ab70a9e2
md"""
...
# ╔═╡ 4811babd-935c-4bd4-b266-6190e292a742
md"""
...
# ╔═╡ 9190917c-3bbf-4719-b576-9c4ef683cf44
md"""
!!! error "Model fix"
...
# ╔═╡ 9b162a8a-2bbb-4655-bc58-b27af0312c42
model = Chain(
...
# ╔═╡ e659554a-5277-4497-a4b9-d3a856c22810
begin
...
# ╔═╡ 4acdeb67-0337-4373-8f5e-5fe3c9dabd93
md"""
!!! info "Task 2"
...
# ╔═╡ de675cfa-63f4-4992-8e2f-93154d8c2184
md"""
### Answers for Task 2
...
# ╔═╡ a7a20e0d-ce5a-41de-9b15-3ea5ec38766e
md"""
### Test network 
...
# ╔═╡ 45373c78-98c7-4d91-b6aa-061b4bd90635
with_terminal() do
	# Test the model up to flattening step
	x = rand(Float32, 32, 32, 3, 1)  # Example input of shape 32x32x3 (one image)
	model = Chain(
		Conv((5,5), 3=>16, pad=SamePad(), relu),
		MaxPool((2,2)),
		Conv((5,5), 16=>16, pad=SamePad(), relu),
...
# ╔═╡ 67711b26-9075-4e78-8f90-2413e0770e05
md"""
...
# ╔═╡ e4b74dae-e123-4e85-8562-d01d5431d301
md"""
...
# ╔═╡ c21b1cc3-d0db-4e0f-a3f2-f0cf35aebb29
accuracy(x, y) = mean(onecold(model(x), 0:9) .== onecold(y, 0:9))

# ╔═╡ 6cb67293-c404-415b-b69c-9a78f365f813
md"""
### Training
...
with_terminal() do
	...
	for epoch = 1:epochs
		...
		acc = accuracy(validate[1]...)
		push!(correct, acc)
  		println(acc)
	end
	plot(correct, ylim=(0.0, 0.75), 
		legend=:none, title="Accuracy", xlabel="epoch", ylabel="proportion correct")
end

# ╔═╡ e15dc39e-ead9-4a52-b6ac-9a57c68e0429
md"""
### Testing the network
...
# ╔═╡ 099b3afb-1bfd-4dca-a16b-036457e21589
md"""
!!! info "Task 3"
	Partition the test set similarly to the training set.
"""

# ╔═╡ fe9e145a-df98-4221-9e1f-431392eb7e99
md"""
## <your code here>
"""

# ╔═╡ 046fb8cc-0f52-469b-af28-7f8ac3f6e87b
begin
    test_x, test_y = CIFAR10(split=:test)[:]
    test_x = reshape(test_x, 32, 32, 3, :)
    test_labels = onehotbatch(test_y, 0:9)
    test = [(test_x[:, :, :, i], test_labels[:, i])
            for i in partition(1:10000, 1000)]
end

# ╔═╡ aa1627a4-ee03-41f7-9039-5861f54cedd6
md"""
!!! info "Task 4"
...
# ╔═╡ d1ab7d7d-9967-45f6-9dd0-24d4e6401303
md"""
...
# ╔═╡ 8e14fcb7-1fe9-4627-aba4-4bea1c748791
md"""
## <your code here>
"""

# ╔═╡ eb7efb3b-4dfc-48b4-9390-66468915e22b
begin
    sample_count = 10
    rand_indices = rand(1:size(test_x, 4), sample_count)
    rand_test = test_x[:, :, :, rand_indices]
    rand_label = test_y[rand_indices]

    df = DataFrame(round.(model(rand_test), digits=2), Symbol.(rand_label), makeunique=true)

    label_counts = Dict{String, Int}()

    for i in 1:sample_count
        actual_class_name = classes[rand_label[i] + 1]  
        
        if haskey(label_counts, actual_class_name)
            label_counts[actual_class_name] += 1
            column_name = Symbol(actual_class_name * "_" * string(label_counts[actual_class_name] - 1))
        else
            label_counts[actual_class_name] = 1
            column_name = Symbol(actual_class_name)
        end
        
        rename!(df, names(df)[i] => column_name)
    end
    df = hcat(DataFrame("Classes/Actual" => classes), df)

    df
end


# ╔═╡ d36c53a0-e7a9-4214-a98d-33476e713191
md"""
!!! tip
...
# ╔═╡ b518053e-3559-4a20-b198-bb6e68f5b451
@bind inx Slider(1:1:10, default=1)

# ╔═╡ f1a6928e-44a1-4276-a9f4-964bab39062c
begin
    if inx <= length(rand_indices)
        selected_index = rand_indices[inx]
        actual_label = test_y[selected_index]
        predicted_label = onecold(model(reshape(test_x[:, :, :, selected_index], 32, 32, 3, 1)), 0:9)[1]
        checkmark = actual_label == predicted_label ? " ✓" : " ✗"
        label_text = classes[actual_label + 1] * checkmark
       
        plot(colorview(RGB, permutedims(test_x[:, :, :, selected_index], (3, 2, 1))), axis=false)
        annotate!(-4, 1.05, text(label_text, :center, 20, :bold, :black))    end
end


# ╔═╡ bed3fceb-7e9f-4332-a4ce-1de540ce35ea
md"""
...
# ╔═╡ 442191df-ce0b-4155-80ec-8379c365c2bd
md"""
### Overall accuracy
"""

# ╔═╡ c9f003aa-d0e4-4fd6-a048-caf3ef34f989
md"""
We iterate over the entire test set to calculate the overall model accuracy.
"""

# ╔═╡ 9cd57743-e518-4b3e-897a-290307090275
round(mean([accuracy(test[i]...) for i in 1:10]), digits=3)

# ╔═╡ 890bbbbc-5d11-4342-955b-59670b008293
md"""
...
# ╔═╡ 3d72dbd1-f739-4a1e-ba06-243af995d29b
DataFrame(accuracy=(class_correct ./ class_total), class=classes)

# ╔═╡ a9dd0a6c-b0d6-4d06-a2a8-4c1dc15c4761
md"""
...
manifest_format = "2.0"
project_hash = "0d844448eed0dd76238bf6ebc69782fc0784cfde"
